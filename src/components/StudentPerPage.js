...
function StudentPerPage() {
  ...
  const fetchData = async (page) => {
    try {
      const response = await fetch(`http://127.0.0.1:8000/students/:page/:limit?page=${page}&limit=${studentsPerPage}`);
      if (!response.ok) {
  ...
  const handleFormSubmit = async () => {
    try {
      const response = await fetch(`http://127.0.0.1:8000/students/:student_id?student_id=`, {
        method: 'PUT',
        headers: {
